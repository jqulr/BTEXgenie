...
```
> [!Note]
> vis-btex takes the `prodigal_output` directory with `{genome}_kofam_abv_thres.tsv` per input genome instead of --hmmscan for visualizing all KOfam hits on an interested pathway.
